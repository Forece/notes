...
Auto.js 是最近比较流行的一款脚本工具，开发简单，只需要一点点的 Javascript 编程基础就可以进行开发了。基本懂得：
...
> Pro 版本虽然强大，但是针对一些流行的 APP（淘宝、京东等） 做了限制，所以现在市面上常见的脚本都是由 4.1 版本制作而成
...
简单来说，函数就是为了避免重复写代码，将可以复用的代码封装成一个函数，需要复用的时候，直接调用即可。而参数可以看做一个不确定的数值，根据我们的需要将这个数值传入我们封装的代码当中，简单例子：
...
UiSelector 即选择器，用于通过各种条件选取屏幕上的控件，再对这些控件进行点击、长按等动作。
...
View 的"重力"。用于决定View的内容相对于View的位置，可以设置的值为:
...
https://blog.csdn.net/qq_20191467/article/details/113620038
...
https://www.jsjiami.com/article/autojs-encode.html

https://blog.csdn.net/snailuncle2/article/details/115387351