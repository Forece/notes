...
~~~js
// 报错，如果是 var 定义的变量，则会是 undefined（被定义但没有赋值）
console.log(a)
let a = 20;
~~~
...
当 arr[0] 和 arr[1] 两个函数运行的时候，循环已经结束，var 定义的 i 没有被销毁，所以 用 console.log(i) 输出的 i 为 2，那么这两个函数输出的结果应该是一样的，都是 2
...
这种情况必须传入对应参数，如果少传参数，没有传参数的变量为 undefined，如果多传则舍弃多余的变量，如果都不传则会报错。为了解决报错的问题，可以设置默认值。
